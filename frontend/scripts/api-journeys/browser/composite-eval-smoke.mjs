...
const DETAIL_SCREENSHOT_PATH = "/tmp/composite-eval-detail-smoke.png";
const USAGE_SCREENSHOT_PATH = "/tmp/composite-eval-usage-smoke.png";
const FAILURE_SCREENSHOT_PATH = "/tmp/composite-eval-smoke-failure.png";

async function main() {
  const auth = await createAuthenticatedContext();
  const suffix = shortRunId(auth.runId);
  const childOneName = `ui_composite_child_a_${suffix}`;
  const childTwoName = `ui_composite_child_b_${suffix}`;
  const compositeName = `ui_composite_eval_${suffix}`;
  let childOneId = null;
  let childTwoId = null;
  ...
  const apiFailures = [];
  const pageErrors = [];
  const evidence = { composite_name: compositeName };

  try {
    const childOne = await createCodeEval(
      auth.client,
      childOneName,
      "return True",
    );
    childOneId = childOne.id;
    const childTwo = await createCodeEval(
      auth.client,
      childTwoName,
    ...
    const created = await auth.client.post(
      ...
      {
        name: compositeName,
        description: "Composite eval browser smoke.",
        tags: ["api-journey", "composite-ui"],
        child_template_ids: [childOneId, childTwoId],
    ...
    assert(isUuid(compositeId), "Composite create did not return a UUID id.");
    ...
    await page.goto(`${APP_BASE}/dashboard/evaluations/${compositeId}`, {
      waitUntil: "domcontentloaded",
    });

    await waitForVisibleText(page, compositeName, { timeout: 30000 });
    await waitForVisibleText(page, "Composite Configuration", {
      exact: true,
      timeout: 30000,
    });
    await waitForVisibleText(page, "Aggregate child eval scores", {
      exact: true,
    });
    await waitForVisibleText(page, "Weighted Average", { exact: true });
    await waitForVisibleText(page, "Children (2)", { exact: true });
    await waitForVisibleText(page, childOneName, { exact: true });
    await waitForVisibleText(page, childTwoName, { exact: true });
    ...
    await waitForResponsesDuring(
      ...
      "composite usage tab",
      [
        (response) => isUsageResponseFor(response, compositeId, "1"),
        (response) => isUsageResponseFor(response, compositeId, "25"),
      ],
    ...
    await cleanupEvalTemplate(auth.client, compositeId, "composite").catch(
      (error) => {
        caughtError = appendCleanupError(caughtError, error);
      },
    );
    await cleanupEvalTemplate(auth.client, childTwoId, "child two").catch(
      (error) => {
        caughtError = appendCleanupError(caughtError, error);
      },
    );
    await cleanupEvalTemplate(auth.client, childOneId, "child one").catch(
      (error) => {
        caughtError = appendCleanupError(caughtError, error);
      },
    );
...
async function createCodeEval(client, name, codeBody) {
  const created = await client.post(
    apiPath("/model-hub/eval-templates/create-v2/"),
    {
      ...
      pass_threshold: 0.5,
      description: "Composite UI smoke child.",
      tags: ["api-journey", "composite-ui-child"],
    },
  );
...
async function cleanupEvalTemplate(client, templateId, label) {
  ...
  console.error(`cleanup ${label}: ${templateId}`);
}
