...
const SCREENSHOT_PATH =
  process.env.OBSERVE_DIRECT_ID_MULTISELECT_SCREENSHOT ||
  "/tmp/observe-direct-id-multiselect-smoke.png";
...
async function main() {
  ...
  const harnessPath = path.join(
    harnessDir,
    "observe-direct-id-multiselect-smoke.html",
  );
  ...
  page.on("request", (request) => {
    ...
    if (url.pathname === "/tracer/dashboard/filter_values/") {
      return request.respond(jsonResponse(200, { result: { values: [] } }));
    }
  ...
  try {
    await page.goto(
      `${APP_BASE}/.tmp-smoke/observe-direct-id-multiselect-smoke.html`,
      { waitUntil: "domcontentloaded" },
    );
    await page.waitForFunction(() =>
      document.body.textContent.includes("Trace ID"),
    );
    await clickByText(page, "Select values...");
    await page.waitForFunction(() =>
      document.body.textContent.includes("trace-a"),
    );
    await clickByText(page, "trace-a");
    await clickByText(page, "trace-b");
    await page.keyboard.press("Escape");
    await page.waitForFunction(
      () =>
        document.body.textContent.includes("trace-a") &&
        document.body.textContent.includes("trace-b"),
    );
    ...
    assert(
      evidence?.field === "trace_id" &&
        evidence?.operator === "in" &&
        Array.isArray(evidence.value) &&
        evidence.value.includes("trace-a") &&
        evidence.value.includes("trace-b"),
      `Expected multi-value Trace ID filter, got ${JSON.stringify(evidence)}`,
    );
...
<html>
  <head>
    <meta charset="UTF-8" />
    <title>Observe Direct ID Multiselect Smoke</title>
    <style>
      body {
        margin: 0;
